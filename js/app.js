(() => {
  ...
  function wireBooking(){
    ...
    form.addEventListener("submit", (e) => {
      ...
      // Demo: aquí luego se conectará Firebase (hold + payment + webhook).
      const payload = {
  ...
  function toast(text){
    ...
    // Reset color after short time (no animations, simple)
...
`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}">
  ...
  function init(){
    ...
    // Default month to current local month at load
    const now = new Date();
    viewMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    renderCalendar();

    // Clear selection with ESC (handy for demos)
